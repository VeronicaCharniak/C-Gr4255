...
//метод, разворачивающий массив через буферную переменную
void SwapArray(int[] arr)
{
    int buf = 0;
    for(int i=0; i<arr.Length/2;i++)
    {
        //(arr[i], arr[arr.Length-1-i]) = (arr[arr.Length-1-i], arr[i]); - как в Python
        buf = arr[i];
...
//метод, генерирующий новый массив, заполненный в обратном порядке
...
SwapArray(testArray);
Console.WriteLine(DateTime.Now - d1); //определение времени, затраченного на метод
//Print1DArr(newArray);
//Print1DArr(testArray);